 ...
 */
//frida -U -f com.zhuomogroup.ylyk -l .\dexDump.js --no-pause
...
function getFunctionName() {
  ...
  if (getAndroidVersion() > 4) {
    ...
    if (dvmExports.length !== 0) {
      ...
      // if not load libdvm.so, check libart.so
...
function getProcessName() {
  ...
  var fopenFunc = new NativeFunction(fopenPtr, "pointer", [
    "pointer",
    "pointer",
  ]);
  var fgetsPtr = Module.findExportByName("libc.so", "fgets");
  var fgetsFunc = new NativeFunction(fgetsPtr, "int", [
    "pointer",
    "int",
    "pointer",
  ]);
    ...
    Interceptor.attach(hookFunction, {
      onEnter: function (args) {
        ...
        if (dexMagicMatch === true) {
          ...
          var odex_path =
            "/data/data/" + processName + "/" + odex_size + ".odex";
